...
namespace Supercyan.FreeSample
{
    public class SimpleSampleCharacterControl : MonoBehaviour
    {
        ...
        public IEnumerator PerformVaulting(string animName, MatchTargetParameters matchTargetParameters = null,
    Quaternion targetRotation = new Quaternion(), bool shouldRotate = false, bool mirrored = false)
{
    ...
    float animLength = animState.length > 0 ? animState.length : 1f; // ✅ Sécurité si length = 0
    animLength = animState.length * 0.85f; 
    ...
    while (time <= animLength)
    {
        ...
        // ✅ Stopper le MatchTarget après matchTargetTime
    ...
    IsVaulting = false;
    Debug.Log("Vault terminé, kinematic désactivé");
}
    }
}
